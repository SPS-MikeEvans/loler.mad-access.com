<x-mobile-layout title="Sign Return — {{ $job->job_number }}">
    <div class="min-h-screen flex flex-col bg-slate-100" x-data="signaturePad()">
        ...
        <header class="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3">
            ...
            <h1 class="font-semibold text-gray-900">Sign Return of Items</h1>
        </header>
        ...
        <div class="flex-1 px-4 py-5 space-y-5 mobile-bottom-safe">
            ...
            <div class="rounded-3xl border border-slate-200 bg-white shadow-sm p-5">
                ...
                <div class="mt-4 space-y-1">
                    <p class="text-sm font-medium text-gray-700">Items being returned:</p>
            ...
            <form method="POST" action="{{ route('jobs.capture-return', $job) }}" id="sig-form">
                @csrf

                {{-- Signer name --}}
                <div class="rounded-3xl border border-slate-200 bg-white shadow-sm p-4 space-y-2">
                    <label class="block text-sm font-medium text-gray-700">Full name of person collecting items</label>
                    <input
                        type="text"
                        name="signed_by"
                        value="{{ old('signed_by') }}"
                ...
                <div class="rounded-3xl border border-amber-100 bg-amber-50 p-4 mt-4 text-xs text-amber-800">
                    By signing, you confirm receipt of all listed items back from inspection. This signature will be retained for legal and audit purposes for a period of 2 years.
                </div>

                <button
                    type="submit"
                    @click="captureSignature()"
                    class="w-full mt-5 py-4 rounded-2xl font-bold text-base text-white bg-green-600 hover:bg-green-700 transition"
                >
