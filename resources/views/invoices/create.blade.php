<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="POST" action="{{ route('clients.invoices.store', $client) }}" class="space-y-6">
                @csrf

                {{-- Date range, notes, pricing adjustments --}}
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 space-y-4">
                        <h3 class="text-lg font-medium text-gray-900">Generate Invoice</h3>
                        ...
                        <div class="border-t pt-4">
                            <h4 class="text-sm font-medium text-gray-900">Pricing adjustment (optional)</h4>
                            <p class="text-xs text-gray-500 mt-1">Use either Discount % or Fixed Total — not both.</p>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3">
                                <div>
                                    ...
                                    <x-text-input id="discount_percent" name="discount_percent" type="number"
                                        step="0.01" min="0.01" max="99.99" class="mt-1 block w-full"
                                        :value="old('discount_percent')" placeholder="e.g. 10" />
                                    <x-input-error :messages="$errors->get('discount_percent')" class="mt-2" />
                                </div>
                                <div>
                                    <x-input-label for="fixed_total" :value="__('Fixed Total (£)')" />
                                    <x-text-input id="fixed_total" name="fixed_total" type="number"
                                        step="0.01" min="0" class="mt-1 block w-full"
                                        :value="old('fixed_total')" placeholder="e.g. 200.00" />
                                    <x-input-error :messages="$errors->get('fixed_total')" class="mt-2" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {{-- Uninvoiced inspections preview with per-line waiver checkboxes --}}
                @if ($uninvoicedInspections->isNotEmpty())
                    @php($oldWaived = collect(old('waived_inspection_ids', []))->map(fn ($id) => (int) $id)->all())
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            ...
                            <p class="text-xs text-gray-500 mb-3">
                                Tick "Waive" to keep the line on the invoice but charge £0 for it.
                            </p>
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200 text-sm">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">Waive</th>
                                        ...
                                        @foreach ($uninvoicedInspections as $inspection)
                                            <tr class="hover:bg-gray-50">
                                                <td class="px-4 py-2 text-center">
                                                    <input type="checkbox" name="waived_inspection_ids[]"
                                                        value="{{ $inspection->id }}"
                                                        class="h-4 w-4 text-brand-navy border-gray-300 rounded focus:ring-brand-red"
                                                        @checked(in_array((int) $inspection->id, $oldWaived, true)) />
                                                </td>
                                    ...
                                    <tfoot>
                                        <tr class="border-t-2 border-gray-300 bg-gray-50">
                                            <td colspan="5" class="px-4 py-2 text-right font-semibold text-gray-700">Total (all uninvoiced)</td>
