<x-mobile-layout title="Inspection – Step {{ $checkIndex + 1 }} of {{ $total }}">
    ...
    >
        {{-- Progress bar --}}
        <div class="bg-white border-b border-gray-100 px-4 pt-3 pb-3">
            <div class="flex items-center justify-between text-xs text-gray-500 mb-1.5">
                <span class="truncate pr-3">{{ $inspection->kitItem->kitType->name }}</span>
        ...
        {{-- Main content --}}
        <div class="flex-1 overflow-y-auto px-4 py-5 space-y-4 mobile-bottom-safe">

            <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm space-y-5">

                {{-- Category badge --}}
                <span class="inline-block bg-brand-blue/20 text-brand-navy text-xs font-semibold px-3 py-1 rounded-full uppercase tracking-[0.18em]">
                    {{ $currentCheck->check_category }}
                </span>

                {{-- Check text --}}
                <div>
                    <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Inspection Prompt</p>
                    <h2 class="mt-2 text-xl font-bold text-gray-900 leading-snug">
                ...
                <div class="grid grid-cols-1 gap-3">
                    <button
                        ...
                        :class="status === 'pass'
                            ? 'bg-green-500 text-white ring-2 ring-green-600 border-green-500'
                            : 'bg-white border-2 border-gray-200 text-gray-700'"
                        class="flex items-center justify-between gap-3 rounded-2xl px-4 py-4 font-bold text-base transition focus:outline-none"
                    >
                        <span>Pass</span>
                        <span class="text-sm font-semibold opacity-80">Equipment acceptable</span>
                    </button>
                    <button
                        @click="setStatus('fail')"
                        :class="status === 'fail'
                            ? 'bg-red-500 text-white ring-2 ring-red-600 border-red-500'
                            : 'bg-white border-2 border-gray-200 text-gray-700'"
                        class="flex items-center justify-between gap-3 rounded-2xl px-4 py-4 font-bold text-base transition focus:outline-none"
                    >
                        <span>Fail</span>
                        <span class="text-sm font-semibold opacity-80">Defect or unsafe finding</span>
                    </button>
                    <button
                        @click="setStatus('n/a')"
                        :class="status === 'n/a'
                            ? 'bg-gray-500 text-white ring-2 ring-gray-600 border-gray-500'
                            : 'bg-white border-2 border-gray-200 text-gray-700'"
                        class="flex items-center justify-between gap-3 rounded-2xl px-4 py-4 font-bold text-base transition focus:outline-none"
                    >
                        <span>N/A</span>
                        <span class="text-sm font-semibold opacity-80">Not applicable</span>
                    </button>
                </div>
                ...
                <div>
                    ...
                    <textarea
                        ...
                        class="w-full border border-gray-300 rounded-2xl px-3 py-3 text-sm focus:ring-brand-red focus:border-brand-red resize-none"
                    ></textarea>
                </div>
                ...
                <div>
                    <div class="flex items-center justify-between gap-3 mb-2">
                        ...
                        <span class="text-xs text-slate-400" x-text="photos.length ? `${photos.length} attached` : 'None yet'"></span>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-2 mb-3" x-show="photos.length > 0">
                        <template x-for="photo in photos" :key="photo.id">
                            <div class="relative">
                                <img :src="photo.url" class="w-full aspect-square object-cover rounded-xl">
                                <button
                    ...
                    <button
                        ...
                        :disabled="uploading"
                        class="w-full border-2 border-dashed border-gray-300 rounded-2xl py-4 text-sm text-gray-500 font-medium hover:border-brand-red hover:text-brand-navy transition disabled:opacity-50"
                    >
                        Take / Add Photo
                    </button>
                </div>
        ...
        {{-- Navigation --}}
        <div class="sticky bottom-0 bg-white/95 backdrop-blur border-t border-gray-100 px-4 py-3 flex gap-3 mobile-bottom-safe">
            @if($checkIndex > 0)
